...
class LightingDemo(mglw.WindowConfig):
  gl_version = (3, 3)
  title = "COM6025M — Lighting"
  window_size = (800, 600)
  ...
  def __init__(self, **kwargs):
    ...
    self.prog = self.ctx.program(
      vertex_shader=load_shader('week06/shaders/cube.vert'),
      fragment_shader=load_shader('week06/shaders/cube.frag'),
    )
    ...
    img = Image.open('week06/textures/diffuse.png').convert('RGB')
    ...
    self.prog['ambient_strength'].value = 0.15
  ...
  def on_render(self, time, frame_time):
    ...
    self.prog['light_dir'].value = tuple(animated_dir)
...
if __name__ == '__main__':
  mglw.run_window_config(LightingDemo)